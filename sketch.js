...
function draw(){
    ...
    if(gameState===1){
        for(var i=0; i<lives; i++){
            image(heart, (width/2-100)+(i*70), 25, 40, 40)
        }
        //play the main theme
    ...
    if(mouseWentDown("leftButton")&&bombCount>0){ 
        //image(explosionImage,mouseX, mouseY, 300, 300);
        explosion=createSprite(mouseX,mouseY)
        explosion.addImage(explosionImage)
        explosion.scale=0.125
        ...
        explosion.overlap(zombiesGroup,(explosion1, zombie1)=>{
            zombie1.destroy()
        })
    }
